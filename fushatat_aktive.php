   ...
   <div class="one">
        ...
        //  $statusi_op=perllogaritHartaAktive($crud,$employee_id);
            if($level=='Supervisor' || $level=='Admin' || $level=='Super_Admin'){
               $query = "SELECT DISTINCT hartat.fushata as fushatat, fushatat.fushata as emri from hartat inner join fushatat on hartat.fushata=fushatat.fushata_id WHERE fushatat.statusi ='attivo'";
                ...
                $w=0;
                 foreach ($result as $key => $res) { 
                      ...
                      ?>
                      <a href='gestione.php?fushatat=<?php echo $fushata?>'  class="btn btn-danger" id="butoni_ri<?php echo $a?>"> <?php echo $emri?></a>
             <?php $w++;} ?>
                            <?php }else{ 
                  echo $level;
                  ?>   
